...
public final class Utils {
    ...
    private static Map<String, String> decodeQuery(final String q, final boolean decodeNames,
        ...
        final Map<String, String> queryParameters = new HashMap<>(16);
    ...
    public static String formatQueryString(final HttpServletRequest request) throws UnsupportedEncodingException {
        String queryString = (request.getQueryString() == null || request.getQueryString().isEmpty()) ? "" : ("?" + request.getQueryString());
        queryString = queryString.replaceAll("\\{", URLEncoder.encode("{", ENCODE_UTF8))
                .replaceAll("}", URLEncoder.encode("}", ENCODE_UTF8))
                .replaceAll("\\|", URLEncoder.encode("|", ENCODE_UTF8));
        return queryString;
    }
}
