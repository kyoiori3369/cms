...
import org.itachi.codestar.util.LocaleUtil;
        ...
        if (messages != null && !messages.isEmpty()) {
            ...
            for (ConstraintViolation<?> message : messages) {
                ...
                if (message.getMessageTemplate() != null && message.getMessageTemplate().length() > 0) {
                    MessageInterpolatorContext context = new MessageInterpolatorContext(message.getConstraintDescriptor(), message.getInvalidValue(), message.getRootBeanClass(), new HashMap<>(0), new HashMap<>(0));
